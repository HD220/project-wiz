...
function SettingsSectionLink({
  ...
  const content = (
    ...
    >
      <div
        className={`mt-1 p-2 rounded-md ${disabled ? "bg-slate-200 dark:bg-slate-700" : "bg-slate-100 dark:bg-slate-800"}`}
      >
        {iconProp &&
          React.createElement(iconProp, {
            className: `h-6 w-6 ${disabled ? "text-slate-400 dark:text-slate-500" : "text-slate-600 dark:text-slate-300"}`,
          })}
      </div>
      <div>
...
function SettingsPage() {
  const settingsSections: SettingsSectionProps[] = [
    ...
    },
    {
      ...
      description:
        "Configure e gerencie suas conexões com Modelos de Linguagem (LLMs).",
    },
    {
    ...
    },
    {
      ...
      description:
        "Gerencie configurações de segurança, autenticação e dados da conta.",
      // Placeholder
      disabled: true,
    },
  ];
