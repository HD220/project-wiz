...
class IPCService {
  ...
  private createMockAPI(): IElectronIPC {
    const mockInvoke = async <T>(
      ...
      throw new Error(
        `MockIPC: Channel '${channel}' not implemented for invoke.`
      );
    };
    ...
    const mockOn = (
      ...
      console.warn(
        `[MockIPC] Listener registered for channel: '${channel}'`,
        listener
      );
      if (channel === IPC_CHANNELS.CHAT_STREAM_EVENT) {
        ...
        setTimeout(
          () => listener({ type: "end" } as ChatStreamEndPayload),
          300
        );
      }
      ...
      return () => {
        console.warn(
          `[MockIPC] Listener removed for channel: '${channel}'`,
          listener
        );
      };
    };
    ...
    return {
      ...
      removeListener: (
        ...
        console.warn(
          `[MockIPC] removeListener called for '${channel}'`,
          listener
        );
      },
  ...
  public onChatStreamEvent(
    ...
    return this.on(
      IPC_CHANNELS.CHAT_STREAM_EVENT,
