...
import type {
  Participant,
  StatusIndicatorMap,
} from "./ProjectParticipantsSidebar";
...
export function ParticipantListItem({
  ...
  const statusInfo = participant.status
    ? statusIndicatorMap[participant.status]
    : null;
    ...
    >
          ...
          <TooltipProvider delayDuration={100}>
            <Tooltip>
              <TooltipTrigger asChild>
                <span
                  className={`absolute right-0 bottom-0 block h-2.5 w-2.5 rounded-full ring-1 ring-background ${statusInfo.color}`}
                />
              </TooltipTrigger>
              <TooltipContent side="left" className="text-xs p-1">
                {statusInfo.icon && (
                  <statusInfo.icon className="h-3 w-3 mr-1 inline-block" />
                )}
                {statusInfo.label}
              </TooltipContent>
            </Tooltip>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p
          ...
          title={participant.roleOrPersona}
        >
          {participant.roleOrPersona ||
            (participant.type === "agent" ? "Agente IA" : "Membro")}
        </p>
      </div>
    </div>
  );
