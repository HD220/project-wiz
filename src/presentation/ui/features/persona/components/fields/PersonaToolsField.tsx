...
export function PersonaToolsField({
  control,
  currentToolInput,
  setCurrentToolInput,
  handleAddTool,
  handleRemoveTool,
}: PersonaToolsFieldProps) {
  return (
    <FormItem>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button
          ...
          disabled={!currentToolInput.trim()}
        >
          Adicionar
        </Button>
      </div>
      <FormDescription>
        Liste as ferramentas que esta persona poderá utilizar (pressione Enter
        ou clique Adicionar). Exemplos:{" "}
        {AVAILABLE_TOOLS.slice(0, 3)
          .map((tool) => tool.id)
          .join(", ")}
        ...
      </FormDescription>
