...
describe("Agent Runtime Module", () => {
  ...
  beforeEach(() => {
    ...
    cqrsDispatcher.registerQueryHandler(
      "GetPersonaQuery",
      async (query: GetPersonaQuery): Promise<Persona | undefined> => {
  ...
  it("should start and stop the agent process", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining(
        "Agent process for Persona test-persona-id started.",
      ),
    );
    expect(consoleSpy).toHaveBeenCalledWith(
      expect.stringContaining(
        "Agent process for Persona test-persona-id stopped.",
      ),
    );
  ...
  it("should log an error and stop if persona cannot be loaded", async () => {
    ...
    const invalidAgentProcess = new AgentProcess(
      "invalid-persona-id",
      cqrsDispatcher,
    );
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      expect.stringContaining("Failed to load persona invalid-persona-id"),
    );
