...
import { GitWorktreeService } from "../../modules/git-integration/domain/git-worktree.service";

import { PersonaProps } from "../../modules/persona-management/domain/persona.entity";

interface WorkerData {
  agentId: string;
  agentConfig: PersonaProps; // Use PersonaProps directly
  task: {
    id: string;
    title: string;
    description?: string;
...
class AgentWorker {
  ...
  >();
  private gitWorktreeService: GitWorktreeService; // Add GitWorktreeService instance

  constructor(data: WorkerData) {
    this.workerData = data;
    this.gitWorktreeService = new GitWorktreeService(this.workerData.projectPath); // Initialize GitWorktreeService
  ...
  private async analyzeTask(
    ...
    agentConfig: WorkerData["agentConfig"],
  ): Promise<string> {
    const prompt = `
  ...
  private async callTool(toolName: string, args: any): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      parentPort?.postMessage({
        type: "tool_call",
        tool: toolName,
        args: { ...args, projectPath: this.workerData.projectPath }, // Pass projectPath
        id,
      });
