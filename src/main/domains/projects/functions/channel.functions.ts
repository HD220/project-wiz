...
export async function createChannel(data: {
  ...
  try {
    ...
    const saved = await db.insert(ChannelTable).values(channel.toPlainObject()).returning();
...
export async function findChannelById(id: string): Promise<any | null> {
  try {
    const db = getDatabase();
    const results = await db.select().from(ChannelTable).where(eq(ChannelTable.id, id));
...
export async function findChannelsByProject(projectId: string): Promise<any[]> {
  try {
    const db = getDatabase();
    const results = await db.select().from(ChannelTable)
...
export async function findAccessibleChannels(projectId: string, userId: string): Promise<any[]> {
  try {
    ...
    logger.info(`Found ${accessibleChannels.length} accessible channels for user: ${userId}`);
    return accessibleChannels;
  } catch (error) {
    logger.error("Failed to find accessible channels", { error, projectId, userId });
    throw error;
  }
}

export async function createGeneralChannel(projectId: string, createdBy: string): Promise<any> {
  try {
    ...
    logger.error("Failed to create general channel", { error, projectId, createdBy });
    throw error;
  }
}
