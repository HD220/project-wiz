...
export class DrizzleLlmConfigRepository implements ILlmConfigRepository {
  ...
  async findById(id: string): Promise<LlmConfig | undefined> {
    try {
      ...
      throw new Error(`Failed to find LLM config by ID: ${(error as Error).message}`);
    }
  }

  async findByProviderAndModel(provider: string, model: string): Promise<LlmConfig | undefined> {
    try {
      const result = await db.select().from(llmConfigs).where(
      ...
      console.error(`Failed to find LLM config by provider ${provider} and model ${model}:`, error);
      throw new Error(`Failed to find LLM config by provider and model: ${(error as Error).message}`);
    }
  }
  ...
  async findAll(): Promise<LlmConfig[]> {
    try {
      ...
      return results.map(data => new LlmConfig({
      ...
      throw new Error(`Failed to find all LLM configs: ${(error as Error).message}`);
    }
  }
