...
export class DrizzleUserSettingsRepository implements IUserSettingsRepository {
  ...
  async findByUserIdAndKey(userId: string, key: string): Promise<UserSetting | undefined> {
    try {
      const result = await db.select().from(userSettings).where(
      ...
      console.error(`Failed to find user setting by user ID ${userId} and key ${key}:`, error);
      throw new Error(`Failed to find user setting by user ID and key: ${(error as Error).message}`);
    }
  }
  ...
  async findByUserId(userId: string): Promise<UserSetting[]> {
    try {
      ...
      console.error(`Failed to find user settings by user ID ${userId}:`, error);
      throw new Error(`Failed to find user settings by user ID: ${(error as Error).message}`);
    }
  }
}
