...
describe("Code Analysis Module", () => {
  ...
  beforeAll(() => {
    ...
    cqrsDispatcher.registerQueryHandler("AnalyzeProjectStackQuery", analyzeProjectStackQueryHandler.handle.bind(analyzeProjectStackQueryHandler));
  });

  it("should analyze project stack for a react project", async () => {
    const query = new AnalyzeProjectStackQuery({ projectPath: "/path/to/react-project" });
    const projectStack = await cqrsDispatcher.dispatchQuery<AnalyzeProjectStackQuery, ProjectStack>(query);

    expect(projectStack.languages).toEqual({ TypeScript: 0.8, JavaScript: 0.2 });
  ...
  it("should analyze project stack for a node project", async () => {
    const query = new AnalyzeProjectStackQuery({ projectPath: "/path/to/node-project" });
    const projectStack = await cqrsDispatcher.dispatchQuery<AnalyzeProjectStackQuery, ProjectStack>(query);
  ...
  it("should return unknown for unhandled project types", async () => {
    const query = new AnalyzeProjectStackQuery({ projectPath: "/path/to/unknown-project" });
    const projectStack = await cqrsDispatcher.dispatchQuery<AnalyzeProjectStackQuery, ProjectStack>(query);
