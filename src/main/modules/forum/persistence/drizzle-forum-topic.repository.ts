...
export class DrizzleForumTopicRepository implements IForumTopicRepository {
  async save(topic: ForumTopic): Promise<ForumTopic> {
    try {
      ...
      throw new Error(`Failed to save forum topic: ${(error as Error).message}`);
    }
  }
  ...
  async findById(id: string): Promise<ForumTopic | undefined> {
    try {
      ...
      throw new Error(`Failed to find forum topic by ID: ${(error as Error).message}`);
    }
  }
  ...
  async findAll(): Promise<ForumTopic[]> {
    try {
      ...
      return results.map(data => new ForumTopic({
        title: data.title,
      ...
      throw new Error(`Failed to find all forum topics: ${(error as Error).message}`);
    }
  }
  ...
  async delete(id: string): Promise<boolean> {
    try {
      ...
      throw new Error(`Failed to delete forum topic: ${(error as Error).message}`);
    }
  }
}