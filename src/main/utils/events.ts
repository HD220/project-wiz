...
export type DomainEvent = 
  ...
  | { type: 'user.preferences.updated'; data: { userId: string; preferences: Record<string, any> } }
  
  // Project events
  | { type: 'project.created'; data: { projectId: string; name: string; ownerId: string } }
  ...
  // Agent events
  | { type: 'agent.created'; data: { agentId: string; name: string; role: string; createdBy: string } }
  | { type: 'agent.updated'; data: { agentId: string } }
  | { type: 'agent.status_changed'; data: { agentId: string; status: string } }
  | { type: 'agent.added_to_project'; data: { agentId: string; projectId: string; role: string } }
  | { type: 'agent.removed_from_project'; data: { agentId: string; projectId: string } }
  
  // Message events
  | { type: 'message.sent'; data: { messageId: string; channelId?: string; dmConversationId?: string; authorId: string; authorType: string } }
  | { type: 'message.edited'; data: { messageId: string; content: string } }
  | { type: 'message.deleted'; data: { messageId: string } }
  
  // Channel events
  | { type: 'channel.created'; data: { channelId: string; projectId: string; name: string } }
  | { type: 'channel.updated'; data: { channelId: string } }
  | { type: 'channel.deleted'; data: { channelId: string } }
  
  // Issue events
  | { type: 'issue.created'; data: { issueId: string; projectId: string; title: string } }
  | { type: 'issue.updated'; data: { issueId: string } }
  | { type: 'issue.status_changed'; data: { issueId: string; oldStatus: string; newStatus: string } }
  | { type: 'issue.assigned'; data: { issueId: string; assigneeId: string; assigneeType: string } }
...
class EventBus extends EventEmitter {
  ...
  subscribe<T extends DomainEvent['type']>(
    eventType: T,
    handler: (data: Extract<DomainEvent, { type: T }>['data']) => void | Promise<void>
  ): () => void {
...
export function publishEvent<T extends DomainEvent['type']>(
  eventType: T,
  data: Extract<DomainEvent, { type: T }>['data']
): void {
...
export function subscribeToEvent<T extends DomainEvent['type']>(
  eventType: T,
  handler: (data: Extract<DomainEvent, { type: T }>['data']) => void | Promise<void>
): () => void {
