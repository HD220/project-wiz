...
const processLLMJob: ProcessorFunction<any, string> = async ({ id, data }) => {
...
export async function exampleQueueUsage() {
  ...
  for (let i = 1; i <= 25; i++) {
    ...
    const job = await llmQueue.add(jobData, {
      priority: i <= 5 ? 10 : 1, // High priority for first 5 jobs
      attempts: 3,
      delay: i > 20 ? 2000 : 0, // Delay last 5 jobs by 2 seconds
    });

    jobs.push(job);
    logger.debug(`Added job ${i} with ID: ${job.id}`);
  }
  ...
  const statsInterval = setInterval(async () => {
    ...
    if (queueStats.waiting === 0 && queueStats.active === 0 && queueStats.delayed === 0) {
  ...
  logger.info(`Cleaned up ${cleanedCompleted} completed and ${cleanedFailed} failed jobs`);
...
export async function performanceTest() {
  ...
  const worker = new Worker("performance-test", async ({ id }) => {
  ...
  const checkCompletion = setInterval(async () => {
    ...
    if (stats.waiting === 0 && stats.active === 0) {
      ...
      logger.info(`Throughput: ${(100 / duration * 1000).toFixed(2)} jobs/second`);
