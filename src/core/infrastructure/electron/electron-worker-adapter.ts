...
export class ElectronWorkerAdapter implements IWorkerService {
  ...
  on(event: 'response' | 'error' | 'progress', listener: (...args: any[]) => void): void {
    // Placeholder para registro de eventos IPC
  }

  /**
   * Executa um prompt via worker Electron, com tratamento de erros e streaming.
   */
  async executePrompt(
    prompt: Prompt,
  ...
  /**
   * Implementa comunicação real via IPC, retornando um stream de chunks.
   */
