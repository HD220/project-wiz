// src/core/domain/job/job-state.mutator.ts
import { DomainError } from "@/core/domain/common/common-domain.errors";
...
export class JobStateMutator<P, R> {
  ...
  constructor(
    ...
    this._conversationHistory =
      initialConversationHistory || ActivityHistoryVO.create();
  ...
  public moveToActive(workerId: string, lockUntil: Date): void {
    if (
      ...
      throw new DomainError(
        `Job ${this.props.id.value} cannot be moved to active from status ${this.props.status}`
      );
    }
  ...
  public extendLock(newLockUntil: Date, workerId: string): void {
    if (
      ...
      throw new DomainError(
        `Cannot extend lock for job ${this.props.id.value}: not active by worker ${workerId}.`
      );
    }
  ...
  public markAsCompleted(returnValue: R): void {
    if (this.props.status !== JobStatus.ACTIVE) {
      console.warn(
        `Job ${this.props.id.value} attempted to complete but was not in ACTIVE state (current: ${this.props.status}).`
      );
    }
  ...
  public markAsFailed(reason: string, stacktrace?: string[]): void {
    if (this.props.status !== JobStatus.ACTIVE) {
      console.warn(
        `Job ${this.props.id.value} attempted to fail but was not in ACTIVE state (current: ${this.props.status}).`
      );
    }
  ...
  public markAsStalled(): boolean {
    ...
    if (this.props.attemptsMade >= this.props.options.attempts) {
      ...
      this.props.failedReason =
        this.props.failedReason ||
        "Job failed after becoming stalled and exceeding max attempts.";
      this.props.finishedOn = new Date();
      return true;
    }
    return false;
  ...
  public pause(): void {
    if (
      ...
      throw new DomainError(
        `Cannot pause job ${this.props.id.value} in status ${this.props.status}.`
      );
    }
  }
  ...
  public resume(): void {
    if (this.props.status === JobStatus.PAUSED) {
      this.props.status =
        this.props.delayUntil && this.props.delayUntil > new Date()
          ? JobStatus.DELAYED
          : JobStatus.WAITING;
      this.props.updatedAt = new Date();
    } else {
      throw new DomainError(
        `Cannot resume job ${this.props.id.value} in status ${this.props.status}.`
      );
    }
  }
