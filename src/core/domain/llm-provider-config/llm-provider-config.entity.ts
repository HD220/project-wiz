// src/core/domain/llm-provider-config/llm-provider-config.entity.ts
import { z } from "zod";
...
const LLMProviderConfigPropsSchema = z.object({
  id: z
    .custom<LLMProviderConfigId>((val) => val instanceof LLMProviderConfigId)
    .optional(),
  name: z.custom<LLMProviderConfigName>(
    (val) => val instanceof LLMProviderConfigName
  ),
...
interface InternalLLMProviderConfigProps
  extends EntityProps<LLMProviderConfigId> {
  name: LLMProviderConfigName;
  providerId: LLMProviderId;
...
export class LLMProviderConfig extends AbstractEntity<
  LLMProviderConfigId,
  InternalLLMProviderConfigProps
> {
  ...
  public static create(
    props: LLMProviderConfigCreationProps
  ): LLMProviderConfig {
    ...
    if (!validationResult.success) {
      ...
      throw new EntityError(
        `Invalid LLMProviderConfig props: ${errorMessages}`,
        {
