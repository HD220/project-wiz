// src/core/application/use-cases/llm-provider-config/create-llm-provider-config.schema.ts
import { z } from "zod";

/**
 * Input schema for CreateLLMProviderConfigUseCase.
 */
export const CreateLLMProviderConfigUseCaseInputSchema = z
  .object({
    name: z
      .string()
      .min(3, {
      ...
      .max(100, {
        message: "Configuration name must be no more than 100 characters long.",
      })
    ...
    providerId: z
      ...
      .max(50, {
        message: "Provider ID must be no more than 50 characters long.",
      })
      .regex(/^[a-z0-9-]+$/, {
        message:
      ...
      .describe(
        "Identifier for the LLM provider (e.g., 'openai', 'anthropic', 'google-gemini')."
      ),
    ...
    baseUrl: z
      ...
      .nullable()
      .describe(
        "Optional custom base URL for the LLM provider API (e.g., for proxies or self-hosted models)."
      ),
...
export type CreateLLMProviderConfigUseCaseInput = z.infer<
  typeof CreateLLMProviderConfigUseCaseInputSchema
>;
...
export const CreateLLMProviderConfigUseCaseOutputSchema = z.object({
  llmProviderConfigId: z
    .string()
    .uuid()
    .describe(
      "The unique identifier of the newly created LLM provider configuration."
    ),
});

export type CreateLLMProviderConfigUseCaseOutput = z.infer<
  typeof CreateLLMProviderConfigUseCaseOutputSchema
>;
