// src/core/application/use-cases/agent/create-agent.schema.ts
import { z } from "zod";

/**
 * Input schema for CreateAgentUseCase.
 */
export const CreateAgentUseCaseInputSchema = z
  .object({
    personaTemplateId: z
      .string()
      .uuid({
    ...
    temperature: z
      ...
      .optional()
      .describe(
        "Optional temperature setting for the agent's LLM. Defaults to a standard value if not provided (e.g., 0.7)."
      ),
    ...
    maxIterations: z
      ...
      .optional()
      .describe(
        "Optional maximum number of iterations for the agent's main loop. Defaults to a standard value if not provided (e.g., 10)."
      ),
...
export type CreateAgentUseCaseInput = z.infer<
  typeof CreateAgentUseCaseInputSchema
>;
...
export const CreateAgentUseCaseOutputSchema = z.object({
  agentId: z
    .string()
    .uuid()
    .describe("The unique identifier of the newly created agent."),
});

export type CreateAgentUseCaseOutput = z.infer<
  typeof CreateAgentUseCaseOutputSchema
>;
