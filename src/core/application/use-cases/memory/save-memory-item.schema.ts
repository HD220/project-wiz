// src/core/application/use-cases/memory/save-memory-item.schema.ts
import { z } from "zod";

/**
 * Input schema for SaveMemoryItemUseCase.
 */
export const SaveMemoryItemUseCaseInputSchema = z
  .object({
    id: z
      .string()
      .uuid({ message: "MemoryItem ID must be a valid UUID if provided." })
      .optional()
      .describe(
        "Optional ID of the memory item to update. If omitted, a new item will be created."
      ),
    ...
    content: z
      ...
      .max(10000, {
        message: "Memory content must be no more than 10000 characters long.",
      })
    ...
    tags: z
      .array(
        z
          ...
          .max(50, {
            message: "Individual tag must be no more than 50 characters.",
          })
      )
      ...
      .optional()
      .describe(
        "Optional array of tags for categorization. Max 20 tags, each 1-50 chars."
      ),
    ...
    source: z
      ...
      .nullable()
      .describe(
        "Optional source of this memory item (e.g., 'user_interaction', 'file_analysis')."
      ),
    ...
    embedding: z
      ...
      .refine((val) => val === null || val === undefined || val.length > 0, {
        message:
          "Embedding array cannot be empty if provided. Use null or omit for no embedding.",
      })
      .describe("Optional pre-computed embedding vector for the content."),
  })
  .strict();

export type SaveMemoryItemUseCaseInput = z.infer<
  typeof SaveMemoryItemUseCaseInputSchema
>;
...
export const SaveMemoryItemUseCaseOutputSchema = z.object({
  memoryItemId: z
    .string()
    .uuid()
    .describe("The ID of the saved (created or updated) memory item."),
  createdAt: z
    .string()
    .datetime()
    .describe("The ISO 8601 date and time when the memory item was created."),
  updatedAt: z
    .string()
    .datetime()
    .describe(
      "The ISO 8601 date and time when the memory item was last updated."
    ),
});

export type SaveMemoryItemUseCaseOutput = z.infer<
  typeof SaveMemoryItemUseCaseOutputSchema
>;
