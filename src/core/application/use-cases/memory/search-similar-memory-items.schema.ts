// src/core/application/use-cases/memory/search-similar-memory-items.schema.ts
import { z } from "zod";
 ...
 */
export const SearchSimilarMemoryItemsUseCaseInputSchema = z
  .object({
    queryEmbedding: z
      .array(z.number())
...
export type SearchSimilarMemoryItemsUseCaseInput = z.infer<
  typeof SearchSimilarMemoryItemsUseCaseInputSchema
>;
...
export const SimilarMemoryListItemSchema = MemoryListItemSchema.extend({
  relevanceScore: z
    ...
    .optional()
    .describe(
      "Optional relevance score indicating similarity (higher is typically better). Exact range depends on the search provider."
    ),
});
...
export const SearchSimilarMemoryItemsUseCaseOutputSchema = z.object({
  items: z
    .array(SimilarMemoryListItemSchema)
    .describe(
      "Array of memory list items found to be similar, ordered by relevance if available."
    ),
});

export type SearchSimilarMemoryItemsUseCaseOutput = z.infer<
  typeof SearchSimilarMemoryItemsUseCaseOutputSchema
>;
