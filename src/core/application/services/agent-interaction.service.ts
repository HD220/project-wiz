...
export class AgentInteractionService {
  ...
  public async performLlmInteraction(
    ...
    if (!llmGenerationResponse.success) {
      this.logger.error(
        `[AgentInteractionService] LLM Generation Failed for Job ID ${job.id.value}: ${llmGenerationResponse.error?.message}`,
        llmGenerationResponse.error,
      );
      // Depending on desired behavior, you might throw an error or set a flag in state
      state.criticalErrorEncounteredThisTurn = true;
      job.addLog(`LLM Generation Failed: ${llmGenerationResponse.error?.message}`, 'ERROR');
      return;
    }
