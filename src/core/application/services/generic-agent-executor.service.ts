// src/core/application/services/generic-agent-executor.service.ts
...
export class GenericAgentExecutor implements IAgentExecutor {
  constructor(
    @inject(AGENT_REPOSITORY_INTERFACE_TYPE)
    private readonly agentRepository: IAgentRepository,
    @inject(LOGGER_INTERFACE_TYPE) private readonly logger: ILogger,
    @inject(AgentInteractionService)
    private readonly agentInteractionService: AgentInteractionService,
    @inject(AgentToolService)
    private readonly agentToolService: AgentToolService,
    @inject(AgentStateService)
  ...
  public async process(
    ...
    this.logger.info(`Processing Job ID: ${jobId} with Agent ID: ${agentId}`, {
      jobId,
      agentId,
    });
    ...
    const executionState = this.agentStateService.initializeExecutionState(
      job,
      agent
    );

    this.logger.info(
      `Job ID: ${jobId} processing attempt: ${job.attemptsMade}`
    );
    job.updateProgress(10);
    this.logger.info(
      `Max iterations for Job ID: ${jobId} set to ${executionState.maxIterations}`
    );
  ...
  private async _executionLoop(
    ...
    while (this._shouldContinueExecution(executionState)) {
      ...
      this.logger.info(
        `Starting LLM interaction cycle ${executionState.iterations} for Job ID: ${job.id.value}`
      );
      job.updateProgress(
        10 + (80 * executionState.iterations) / executionState.maxIterations
      );

      await this.agentInteractionService.performLlmInteraction(
        job,
        agent,
        executionState
      );
      ...
      if (
        executionState.assistantMessage?.tool_calls &&
        executionState.assistantMessage.tool_calls.length > 0
      ) {
        await this.agentToolService.handleToolCallsIfPresent(
          job,
          agent,
          executionState
        );
  ...
  private _shouldContinueExecution(state: ExecutionState): boolean {
    return (
      ...
      !state.goalAchieved &&
      !state.criticalErrorEncounteredThisTurn
    );
  }

  private _handleEndOfLoopConditions(
    job: JobEntity<AgentExecutionPayload, unknown>,
    executionState: ExecutionState
  ): boolean {
    if (executionState.goalAchieved) {
      this.logger.info(
        `Goal achieved for Job ID: ${job.id.value} in iteration ${executionState.iterations}.`
      );
      return true;
    }
  ...
  private _constructFinalResult(
    ...
    if (state.goalAchieved) {
      return this._createJobProcessingOutput(
        job,
        state,
        AgentExecutorStatus.SUCCESS,
        `Goal achieved. Last LLM response: ${state.llmResponseText}`
      );
    }
    if (state.criticalErrorEncounteredThisTurn) {
      const lastErrorEntry = state.executionHistory
        .slice()
        .reverse()
        .find((entry) => entry.type.endsWith("_error"));
      const errorMessage = lastErrorEntry?.error
        ? String(lastErrorEntry.error)
  ...
  private _createJobProcessingOutput(
    ...
    return {
      ...
      errors: state.executionHistory.filter((entry) =>
        entry.type.endsWith("_error")
      ),
    };
  }

  private _getSerializableHistory(
    job: JobEntity<AgentExecutionPayload, unknown>
  ) {
