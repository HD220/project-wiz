// src/infrastructure/queue/drizzle/queue-maintenance.service.ts
    ...
    if (this.maintenanceLoopPromise) {
      try {
        ...
        this.eventEmitter.emit(
          "queue.error",
          new Error(
            `Maintenance loop stop error: ${error instanceof Error ? error.message : String(error)}`
          )
        );
      }
    }
    ...
    while (this._isMaintenanceRunning) {
      try {
        ...
        this.eventEmitter.emit(
          "queue.error",
          new Error(
            `Stalled job maintenance: ${error instanceof Error ? error.message : String(error)}`
          )
        );
      }
