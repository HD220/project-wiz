// src/examples/queue-usage-example.final.ts
...
import { DrizzleQueueFacade as QueueService } from "@/infrastructure/queue/drizzle/drizzle-queue.service";
...
function setupEventListeners(
  ...
  queue.on("job.active", (job) =>
    process.stdout.write(
      `[Queue] Job ${job.getProps().id.value} is now active.\n`
    )
  );
  queue.on("job.stalled", (job) =>
    process.stdout.write(
      `[Queue] Job ${job.getProps().id.value} stalled and re-queued.\n`
    )
  );
...
async function main() {
  const jobRepository = new DrizzleJobRepository<
    ...
    EmailJobResult
  >(db);
  const coreService = new QueueServiceCore("default", jobRepository);
