...
export function useForum() {
  const {
    ...
    error,
    refetch: fetchTopics,
  } = useIpcQuery<IForumTopic[], IpcForumListTopicsPayload>({
    channel: IpcChannel.FORUM_LIST_TOPICS,
  });

  const { mutate: createTopic } = useIpcMutation<
    IForumTopic,
    Error,
    IpcForumCreateTopicPayload
  >({
  ...
  const { mutate: createPost } = useIpcMutation<
    IForumPost,
    Error,
    IpcForumCreatePostPayload
  >({
  ...
  const {
    ...
    error: postsError,
    refetch: fetchPosts,
  } = useIpcQuery<IForumPost[], IpcForumListPostsPayload>({
  ...
  const handleGetPostsForTopic = async (
    topicId: string,
  ): Promise<IForumPost[]> => {
