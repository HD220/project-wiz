...
export function useMessages(conversationId?: string, limit?: number, offset?: number) {
  const currentConversationMessages = useMessageStore((state: any) => state.currentConversationMessages);
  const setCurrentConversationMessages = useMessageStore((state: any) => state.setCurrentConversationMessages);
