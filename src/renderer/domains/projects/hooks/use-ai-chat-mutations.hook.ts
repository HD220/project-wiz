...
export function useAiChatMutations(channelId: string) {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: (data: AISendMessageRequestDto) => aiChatService.sendMessage(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['channelMessages', 'channel', channelId] });
    },
  });

  const regenerateMessageMutation = useMutation({
    mutationFn: (data: AIRegenerateMessageRequestDto) => aiChatService.regenerateMessage(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['channelMessages', 'channel', channelId] });
    },
  });
