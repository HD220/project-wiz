...
export function useChannelChat(
  props: UseChannelChatProps,
): UseChannelChatReturn {
  const {
    channelId,
    llmProviderId,
    authorId,
  ...
  const sendMessage = useCallback(
    async (
      ...
      try {
        ...
        const response: AISendMessageResponseDto =
          await window.electronIPC.invoke(
  ...
  const regenerateLastMessage = useCallback(
    async (customConfig?: Partial<AIChatConfigDto>): Promise<void> => {
      if (!llmProviderId) {
        throw new Error("LLM Provider não configurado");
      }
      ...
      try {
        ...
        const regeneratedMessage: ChannelMessageDto =
          await window.electronIPC.invoke(
  ...
  const getConversationSummary = useCallback(
    async (messageLimit?: number): Promise<string> => {
      try {
        return await window.electronIPC.invoke(
  ...
  const updateConfig = useCallback(
    (newConfig: Partial<UseChannelChatProps>) => {
