...
export function useDirectMessageChat({ conversationId }: UseDirectMessageChatProps): UseDirectMessageChatReturn {
  ...
  const { messages: dbMessages, isLoading, createMessage, loadMessages } = useMessages(conversationId);

  // Combinar mensagens do banco com otimistas, mas apenas para a conversa atual
  const messages = [...dbMessages, ...optimisticMessages.filter(msg => msg.conversationId === conversationId)];
  ...
  const sendMessage = useCallback(async (content: string): Promise<void> => {
    ...
    try {
      ...
      const response = await window.electronIPC.invoke("dm:agent:sendMessage", {
        conversationId,
  ...
  const regenerateLastMessage = useCallback(async (): Promise<void> => {
    ...
    try {
      const regeneratedMessage = await window.electronIPC.invoke("dm:agent:regenerateResponse", {
        conversationId,
      });
