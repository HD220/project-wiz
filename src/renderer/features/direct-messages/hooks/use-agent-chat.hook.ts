...
export const useAgentChat = ({ conversationId, conversation }: UseAgentChatProps) => {
  ...
  return {
    ...
    validateAgent: async () => {
      try {
        return await window.electronIPC.invoke("dm:agent:validateConversation", { conversationId });
      } catch (error) {
