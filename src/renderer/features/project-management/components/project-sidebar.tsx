  ...
  const handleRemoveProject = async (id: string) => {
    try {
      ...
      if (result.success) {
        ...
        alert(`Error removing project: ${result.error?.message || 'An unknown error occurred'}`);
      }
    } catch (err: unknown) {
    ...
    <div className="w-64 bg-gray-100 p-4 border-r border-gray-200">
      ...
      <ul>
        {projects.map((project) => (
          <li key={project.id} className="flex justify-between items-center py-2 border-b border-gray-200 last:border-b-0">
