// src_refactored/core/application/ports/job-repository.interface.ts
...
export interface IJobRepository<P extends { userId?: string }, R = unknown> {
   ...
   */
  acquireLock(jobId: JobIdVO, workerId: string, lockUntil: Date): Promise<boolean>;
   ...
   */
  countJobsByStatus(queueName: string, statuses?: JobStatus[]): Promise<Partial<Record<JobStatus, number>>>;
   ...
   */
  clean(queueName: string, gracePeriodMs: number, limit: number, status?: JobStatus): Promise<number>;
}
