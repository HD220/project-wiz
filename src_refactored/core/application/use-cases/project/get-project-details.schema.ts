// src_refactored/core/application/use-cases/project/get-project-details.schema.ts
import { z } from 'zod';
 ...
 */
export const GetProjectDetailsUseCaseInputSchema = z.object({
  projectId: z.string().uuid({ message: "Project ID must be a valid UUID." }),
}).strict();

export type GetProjectDetailsUseCaseInput = z.infer<typeof GetProjectDetailsUseCaseInputSchema>;

/**
 * Schema for the source code details part of the output.
 */
export const SourceCodeDetailsSchema = z.object({
  id: z.string().uuid().describe("The unique identifier of the source code entry."),
  repositoryPath: z.string().describe("The file system path to the project's source code repository."),
  // docsPath can be undefined in the SourceCode entity, so it's nullable in the DTO.
  docsPath: z.string().nullable().describe("The file system path to the project's documentation, or null if not set."),
});
...
export const GetProjectDetailsUseCaseOutputSchema = z.object({
  ...
  sourceCode: SourceCodeDetailsSchema.nullable().describe("Details of the project's source code, or null if not available."),
});

export type GetProjectDetailsUseCaseOutput = z.infer<typeof GetProjectDetailsUseCaseOutputSchema>;
