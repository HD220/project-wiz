// src_refactored/core/application/use-cases/memory/search-memory-items.schema.ts
import { z } from 'zod';

/**
 * Input schema for SearchMemoryItemsUseCase (non-semantic text/tag search).
 */
export const SearchMemoryItemsUseCaseInputSchema = z.object({
...
export type SearchMemoryItemsUseCaseInput = z.infer<typeof SearchMemoryItemsUseCaseInputSchema>;
...
export const SearchMemoryItemsUseCaseOutputSchema = z.object({
  items: z.array(MemoryListItemSchema).describe("Array of memory list items for the current page."),
  totalCount: z.number().int().describe("Total number of memory items matching the filters."),
...
export type SearchMemoryItemsUseCaseOutput = z.infer<typeof SearchMemoryItemsUseCaseOutputSchema>;
