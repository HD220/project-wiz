// src_refactored/infrastructure/queue/drizzle/drizzle-queue.facade.ts
...
export class DrizzleQueueFacade<P extends { userId?: string }, R> extends AbstractQueue<P, R> {
  ...
  async extendJobLock(jobId: string | JobIdVO, workerId: string, lockDurationMs: number): Promise<void> {
    return this.processingService.extendJobLock(jobId, workerId, lockDurationMs);
  }

  async markJobAsCompleted(jobId: string | JobIdVO, workerId: string, result: R, jobInstanceWithChanges: JobEntity<P,R>): Promise<void> {
    return this.processingService.markJobAsCompleted(jobId, workerId, result, jobInstanceWithChanges);
  }

  async markJobAsFailed(jobId: string | JobIdVO, workerId: string, error: Error, jobInstanceWithChanges: JobEntity<P,R>): Promise<void> {
    return this.processingService.markJobAsFailed(jobId, workerId, error, jobInstanceWithChanges);
  }
