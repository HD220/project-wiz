// src_refactored/infrastructure/adapters/llm/mock-llm.adapter.ts
...
import { LLMGenerationOptions, LanguageModelMessage } from '@/core/ports/adapters/llm-adapter.types';

import { Result, Ok, Err } from '@/shared/result';

@injectable()
export class MockLLMAdapter implements ILLMAdapter {
  constructor(@inject(LoggerServiceToken) private readonly logger: ILoggerService) {
  ...
  async generateText(
    ...
    this.logger.info('[MockLLMAdapter] generateText called', { messages, options });
    ...
    if (lastUserMessage?.content?.toLowerCase().includes("use tool")) {
        return Ok({
            ...
            tool_calls: [{
                ...
                function: {
                    name: "example.tool",
                    arguments: JSON.stringify({ message: "hello from mock tool call" })
                }
            }]
        });
    }
  ...
  async generateStructuredOutput<S extends z.ZodTypeAny>(
    ...
    this.logger.info('[MockLLMAdapter] generateStructuredOutput called', { prompt, schemaDef: schema.description, options });
    ...
    try {
      if (schema instanceof z.ZodObject) {
        ...
        for (const key in schema.shape) {
          ...
          if (fieldSchema instanceof z.ZodString) mockData[key] = `mock string for ${key}`;
          else if (fieldSchema instanceof z.ZodNumber) mockData[key] = 123;
          else if (fieldSchema instanceof z.ZodBoolean) mockData[key] = true;
          else mockData[key] = null; // Default for other types
        }
        const validation = schema.safeParse(mockData);
        if (validation.success) return Ok(validation.data);
        return Err(new LLMError('Failed to generate mock structured output matching schema.', validation.error));
      }
    } catch (e) {
        return Err(new LLMError('Error generating mock structured output for schema.', e instanceof Error ? e : undefined));
    }
    return Err(new LLMError('Mock structured output for this schema type not implemented.'));
  }
