...
// UserCircle2 from lucide-react was unused
// Avatar, AvatarFallback, AvatarImage from '@/components/ui/avatar' were unused
// Tooltip, TooltipContent, TooltipProvider, TooltipTrigger from '@/components/ui/tooltip' were unused
...
// Export for use in ParticipantListItem.tsx
...
const mockParticipants: Participant[] = [
  ...
  },
  {
    ...
    type: "agent",
    roleOrPersona: "Eng. Software Sênior",
    status: "running",
  },
  {
    id: "agent002",
  ...
  },
  {
    ...
    name: "Deployatron",
    type: "agent",
    roleOrPersona: "DevOps Specialist",
    status: "error",
  },
];

// Export for use in ParticipantListItem.tsx
export type StatusIndicatorMap = Record<
  Required<Participant>["status"],
  { color: string; label: string; icon?: React.ElementType }
>;

const statusIndicatorMap: StatusIndicatorMap = {
...
export function ProjectParticipantsSidebar({
  className,
}: ProjectParticipantsSidebarProps) {
  ...
  const filteredParticipants = participants.filter(
    (participant) =>
      ...
      (participant.roleOrPersona || "")
        .toLowerCase()
        .includes(searchTerm.toLowerCase())
  );

  // p to person
  const humanParticipants = filteredParticipants.filter(
    (person) => person.type === "human"
  );
  // p to agent
  const agentParticipants = filteredParticipants.filter(
  ...
  return (
    <aside
      className={cn(
        "w-64 flex-shrink-0 bg-slate-100 dark:bg-slate-800/70 border-l border-slate-200 dark:border-slate-700 flex flex-col h-full",
        className
      )}
    >
      ...
      <ScrollArea className="flex-1">
        ...
        {agentParticipants.length > 0 && (
          <div className="p-2 pt-1">
            ...
            <h4 className="text-xs font-semibold text-slate-500 dark:text-slate-400 px-2 my-2 flex items-center">
              <BotIcon className="mr-1.5 h-4 w-4" /> Agentes IA (
