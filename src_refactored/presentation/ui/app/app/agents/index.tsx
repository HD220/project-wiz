...
const mockPersonaTemplates: Record<
  string,
  Pick<PersonaTemplate, "id" | "name">
> = {
...
const mockLlmConfigs: Record<
  string,
  Pick<LLMConfig, "id" | "name" | "providerId">
> = {
...
const mockAgentInstances: AgentInstance[] = [
  {
    id: "agent-001",
    agentName: "CoderBot-Alpha",
    personaTemplateId: "1",
    llmProviderConfigId: "1",
    temperature: 0.7,
    status: "idle",
    currentJobId: null,
  ...
  },
  {
    id: "agent-003",
    personaTemplateId: "1",
    llmProviderConfigId: "2",
    temperature: 0.8,
    status: "error",
    currentJobId: "job-120",
...
function AgentInstancesPage() {
  ...
  const getPersonaName = (templateId: string): string =>
    mockPersonaTemplates[templateId]?.name || "Desconhecido";
  const getLlmConfigName = (configId: string): string =>
    mockLlmConfigs[configId]?.name || "Desconhecido";
  ...
  const filteredInstances = enrichedAgentInstances.filter(
    (instance) =>
      (instance.agentName || "")
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      instance.personaTemplateName
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
        ...
        <div className="text-center py-10 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-lg mt-6">
          ...
          <h3 className="mt-2 text-sm font-medium text-slate-900 dark:text-slate-50">
            {searchTerm
              ...
              : "Nenhuma instância de agente criada"}
          </h3>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            {searchTerm
              ? "Tente ajustar seus termos de busca."
              : "Comece criando sua primeira instância de agente."}
          </p>
          {!searchTerm && (
            <div className="mt-6">
