...
const mockPersonaTemplates: PersonaTemplate[] = [
  { id: '1', name: 'Engenheiro de Software Sênior', role: 'Desenvolvedor especialista em arquiteturas complexas e refatoração de código.', goal: 'Escrever código limpo, eficiente, bem documentado e testado. Mentorar desenvolvedores juniores.', backstory: '15 anos de experiência em diversas linguagens e paradigmas.', toolNames: ['filesystem', 'terminal', 'code-editor', 'search'] },
  { id: '2', name: 'Analista de QA Detalhista', role: 'Especialista em testes de software, focado em encontrar bugs críticos e garantir a qualidade.', goal: 'Garantir que o software atenda aos mais altos padrões de qualidade antes do lançamento. Criar planos de teste abrangentes.', backstory: 'Apaixonado por qualidade e processos de teste desde o início da carreira.', toolNames: ['test-runner', 'issue-tracker', 'browser-dev-tools'] },
  { id: '3', name: 'Gerente de Projetos Ágil', role: 'Facilitador de equipes ágeis, focado em entregas de valor e comunicação eficiente.', goal: 'Manter o projeto nos trilhos, remover impedimentos e garantir a satisfação do cliente e da equipe.', backstory: 'Experiência como Scrum Master e Product Owner em startups e grandes corporações.', toolNames: ['task-manager', 'gantt-chart', 'communication-tools'] },
  { id: '4', name: 'Redator Técnico Criativo', role: 'Escritor especializado em transformar informações técnicas complexas em conteúdo claro e envolvente.', goal: 'Produzir documentação de alta qualidade, tutoriais e guias de usuário que sejam fáceis de entender.', backstory: 'Background em jornalismo e paixão por tecnologia.', toolNames: ['markdown-editor', 'documentation-generator', 'screenshot-tool'] },
];

function PersonaTemplatesPage() {
  const [searchTerm, setSearchTerm] = useState('');

  const filteredTemplates = mockPersonaTemplates.filter(template =>
      ...
      {filteredTemplates.length > 0 ? (
        ...
        <div className="text-center py-10 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-lg mt-6">
          ...
          <h3 className="mt-2 text-sm font-medium text-slate-900 dark:text-slate-50">
            {searchTerm ? 'Nenhum template encontrado' : 'Nenhum template de persona criado'}
          </h3>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
            {searchTerm ? 'Tente ajustar seus termos de busca.' : 'Comece criando seu primeiro template de persona.'}
          </p>
          {!searchTerm && (
            <div className="mt-6">
