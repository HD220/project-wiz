import { LLMConfig, AgentLLM, LLMSettings } from '../../../../shared/types/entities';
...
export const mockAvailableLLMs: LLMSettings[] = [
  ...
  // Add more mock LLMs as needed, including local/Ollama if supported
  {
    ...
    name: 'Ollama Llama 2',
    provider: 'Ollama',
    model: AgentLLM.OLLAMA_LLAMA2, // Assuming this enum value exists
    description: 'Llama 2 running locally via Ollama.',
    apiKeyRequired: false, // Typically no API key for local Ollama
    apiUrl: 'http://localhost:11434/api/generate', // Example Ollama URL
  }
];

// This could represent user-saved global LLM configurations/credentials
export let mockUserLLMConfigs: Record<AgentLLM, Partial<LLMConfig> & { apiKey?: string }> = {
  [AgentLLM.OPENAI_GPT_4_TURBO]: {
    apiKey: 'sk-mockOpenAIKey123', // Mock API key
    temperature: 0.7,
  },
  [AgentLLM.ANTHROPIC_CLAUDE_3_OPUS]: {
    apiKey: 'sk-mockAnthropicKey456', // Mock API key
    temperature: 0.8,
  },
  // User might not have configured all LLMs
};

export const updateUserLLMConfig = (llm: AgentLLM, config: Partial<LLMConfig> & { apiKey?: string }) => {
...
export const getLLMConfigWithDefaults = (llm: AgentLLM): LLMConfig => {
  ...
  const modelDetails = mockAvailableLLMs.find(m => m.model === llm);
  ...
  return {
    ...
    topK: userConfig.topK, // Can be undefined
    ...
    apiKey: userConfig.apiKey, // Can be undefined
    apiBaseUrl: modelDetails?.apiUrl, // Get from available LLMs
  };
};
