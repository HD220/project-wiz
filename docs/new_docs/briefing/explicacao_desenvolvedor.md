...
3.  **Obtenção pelo Agente (Worker):** Um Agente (atuando como seu próprio Worker), que corresponde à `Persona` designada e está ocioso, **solicita ativamente** à `Queue` um `Job` elegível de sua responsabilidade. A elegibilidade considera as dependências e a prioridade, que o próprio Agente pode ter influenciado anteriormente para seus `Jobs` através de `Tools` específicas.
