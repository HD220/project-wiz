...
*   **Ações da PersonaDevSenior (internas, orquestradas pelo seu LLM):**
    ...
    3.  **Estabelecimento da Working Directory e Acesso ao Código:**
        *   A Persona define uma `working-directory` isolada para este Job.
        *   Usa `gitCloneTool(repo_url, working_directory)` (ou `gitFetchTool` se o repo já existir).
        *   Usa `gitCheckoutBranchTool(branch_Y)` na `working-directory`.
        *   Usa `readFileTool(working_directory + "/" + caminho_arquivo_original)` para carregar o código no `ActivityContext`.
    4.  **Análise Inicial e Definição de Pronto (`validationCriteria`):**
        *   LLM analisa o código (similar à Task 1 original).
...
*   A PersonaDevSenior processa seus Sub-Jobs sequencialmente, conforme as dependências:
    ...
    *   **Sub-Job 1: Analisar e Refatorar `modulo.py` para PEP 8**
        ...
        *   **Ação:** Persona usa `writeFileTool(path=working_directory + "/" + caminho_arquivo_original, content=codigo_refatorado_do_llm)` (sobrescreve o original na working directory).
        *   **Ação:** Persona usa `executeTerminalCommandTool(command="flake8 " + caminho_arquivo_original, cwd=working_directory)` para verificar. Se houver erros, pode haver um sub-loop de correção com o LLM e `writeFileTool`. (Nota: Se encontrar dificuldades persistentes, a Persona pode aplicar "regras de ouro": reavaliar a abordagem, dividir mais o problema, ou como último recurso, pedir ajuda ao usuário).

    *   **Sub-Job 2: Identificar Funções Chave em `modulo.py` para Testes**
        *   **Ação (Task interna):** Persona envia o código refatorado (do `ActivityContext` do Sub-Job 1 ou relendo da `working-directory`) ao LLM: "Liste as funções principais deste módulo que deveriam ter testes unitários."
    ...
    *   **Sub-Job 3: Gerar Testes Unitários (`unittest`) para Funções Identificadas**
        *   Para cada função identificada:
            ...
            *   **Ação:** Persona agrega os testes gerados e usa `writeFileTool(path=working_directory + "/test_modulo.py", content=codigo_testes_agregados, mode="append_or_create")`. (Pode usar `searchAndReplaceInFileTool` ou `applyDiffTool` para modificações mais granulares se o arquivo já existir e precisar de ajustes).
        *   *(Suposição: Persona pode usar `findFilesByNameTool("*.py", working_directory)` ou `searchInFileContentTool("def minha_funcao", working_directory)` se precisar localizar/confirmar arquivos ou conteúdo específico).*

    *   **Sub-Job 4: Executar e Validar Testes Unitários**
        *   **Ação:** Persona usa `executeTerminalCommandTool(command="python -m unittest test_modulo.py", cwd=working_directory)`.
    ...
    *   **Sub-Job 5: Commitar Alterações e Preparar Entrega**
        *   **Ação:** Persona usa `gitCreateBranchTool(branch_name="refatoracao/modulo-py-" + timestamp, cwd=working_directory)`.
        *   **Ação:** Persona usa `gitAddTool(files=[caminho_arquivo_original, "test_modulo.py"], cwd=working_directory)`.
        ...
        *   **Ação:** Persona usa `gitCommitTool(message=mensagem_commit, cwd=working_directory)`.
        *   **Ação (Opcional, configurável):** Persona usa `gitPushTool(branch_name="refatoracao/modulo-py-" + timestamp, cwd=working_directory)`.
...
*   **Ações da PersonaDevSenior (Chat):**
    *   "Olá! Concluí a refatoração do `modulo.py` e a criação dos testes unitários.
        *   As alterações foram commitadas no branch: `refatoracao/modulo-py-<timestamp>` na sua `working-directory` em `path/to/working-directory`.
