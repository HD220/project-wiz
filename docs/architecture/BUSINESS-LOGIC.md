...
import { UpdateUserSchema, type UpdateUserInput, type User } from '../../shared/types/user';

export class UserService {
  /**
   * Atualizar perfil do usuário
   */
  static async updateProfile(userId: string, input: UpdateUserInput): Promise<User> {
    // Validação
  ...
  private static async publishUserDeactivatedEvent(userId: string): Promise<void> {
...
import { CreateAgentSchema, type CreateAgentInput, type Agent } from '../../shared/types/agent';
...
export class AgentService {
  ...
  static async create(input: CreateAgentInput, createdBy: string): Promise<Agent> {
    ...
    const newAgent = {
      id: generateId(),
      ...validated,
      systemPrompt: validated.systemPrompt || this.generateDefaultSystemPrompt(validated),
  ...
  private static async validateLLMConfiguration(config: CreateAgentInput): Promise<void> {
...
import { CreateProjectSchema, type CreateProjectInput, type Project } from '../../shared/types/project';
...
export class ProjectService {
  ...
  // Métodos de eventos
  private static async publishProjectCreatedEvent(project: Project): Promise<void> {
  ...
  private static async publishProjectUpdatedEvent(project: Project): Promise<void> {
  ...
  private static async publishProjectArchivedEvent(projectId: string): Promise<void> {
  ...
  private static async publishProjectDeletedEvent(projectId: string): Promise<void> {
...
import { SendMessageSchema, type SendMessageInput, type Message } from '../../shared/types/message';
...
export class ChatService {
  ...
  static async sendMessage(input: SendMessageInput, authorId: string): Promise<Message> {
    ...
    if (validated.channelId) {
      ...
      throw new ValidationError('Either channelId or dmConversationId must be provided');
    }
  ...
  private static async processMessageForActions(message: Message): Promise<void> {
    ...
    try {
      ...
      if (intent.requiresAgent && intent.targetAgentId) {
        ...
        if (channel) {
          const projectAgentsList = await AgentService.listByProject(channel.projectId);
          const appropriateAgent = this.selectAppropriateAgent(projectAgentsList, intent);
  ...
  private static selectAppropriateAgent(
    ...
    for (const agent of agents) {
      if (intent.categories.some(category => 
  ...
  // Métodos de eventos
  private static async publishMessageSentEvent(message: Message): Promise<void> {
...
*Esta camada de business logic foi projetada para ser robusta, testável e escalável, encapsulando todas as regras de negócio do Project Wiz.*