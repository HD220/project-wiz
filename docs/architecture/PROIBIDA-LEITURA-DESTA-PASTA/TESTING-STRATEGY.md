...
describe("ProjectService", () => {
  ...
  describe("create", () => {
    ...
    it("deve falhar com dados inválidos", async () => {
      ...
      // Act & Assert
      await expect(ProjectService.create(invalidInput)).rejects.toThrow(
    ...
    it("deve gerar ID único para cada projeto", async () => {
      ...
      vi.mocked(db.insert).mockReturnValue({
        values: vi.fn().mockReturnValue({
          returning: vi.fn().mockResolvedValue([{ id: "project-1", ...input }]),
        }),
      });
...
test.describe("Project Creation Flow", () => {
  ...
  test("deve criar projeto completo via interface", async ({ page }) => {
    ...
    await expect(page.locator("[data-testid=project-header]")).toContainText(
      "E2E Test Project",
    );
...
test.describe("Agent Chat Flow", () => {
  ...
  test("deve enviar mensagem e receber resposta do agente", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("[data-testid=message-list] [data-author-type=agent]"),
    ).toBeVisible({ timeout: 10000 });

    // Verify agent responded
    const agentMessages = page.locator(
      "[data-testid=message-list] [data-author-type=agent]",
    );
  ...
  test("deve permitir mencionar agente específico", async ({ page }) => {
    ...
    await expect(
      page.locator("[data-testid=message-list] .mention"),
...
_Esta estratégia de testes foi projetada para ser prática, eficiente e adequada para equipes de todos os níveis, garantindo qualidade sem adicionar complexidade desnecessária._
