...
export class PriorityTaskQueue {
  ...
  constructor() {
    ...
    this.heap = new BinaryHeap<PrioritizedTask>((a, b) => {
...
export class PersistentQueue {
  ...
  async dequeue(workerId: string): Promise<QueuedTask | null> {
    ...
    const [task] = await this.db
      ...
      .where(and(eq(queueTable.status, "pending"), isNull(queueTable.workerId)))
  ...
  getStats() {
    const avgProcessingTime =
      this.metrics.processingTime.length > 0
        ? this.metrics.processingTime.reduce((a, b) => a + b) /
    ...
    return {
      ...
      avgProcessingTime,
      successRate:
        this.metrics.processed / (this.metrics.processed + this.metrics.failed),
    };
  }
}
```
